...
class User(db.Model,
           UserMixin):  # UserMixin is a class from Flask-Login that provides the methods needed for user auth.
    ...
    accounts = db.relationship('Account', backref='owner',
                               lazy=True)  # 1-to-many relationship with Account (its here but I dont really think we'll have users with multiple accounts). Backref allows us to access the owner of an account from the account object.
...
class Account(db.Model):
    ...
    weekly_spending_limit = db.Column(db.Float,
                                      nullable=False)  # to store how much money the user wants to spend this week. tbd if this is used.
